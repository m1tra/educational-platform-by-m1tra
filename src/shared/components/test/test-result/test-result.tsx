...
import { Trophy, Info, Clock, CheckCircle, XCircle, BarChart3, Repeat, AlertCircle, ArrowLeft } from "lucide-react"
        ...
        <CardContent className="space-y-6 py-6">
          <Tabs defaultValue="overview" className="w-full">
            <TabsList className="grid grid-cols-4 mb-4">
              ...
              <TabsTrigger value="answer-details">Ответы</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="answer-details" className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
              {answerList.map((answer, index: number) => (
                <div key={index} className="border rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="font-medium">{`${answer.id}. ${answer.task}`}</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        Ваш ответ: <span className="font-medium">{answer.userAnswer}</span>
                      </p>
                      {answer.correctAnswer !== answer.userAnswer && (
                        <p className="text-sm text-green-600 mt-1">
                          Правильный ответ: <span className="font-medium">{answer.correctAnswer}</span>
                        </p>
                      )}
                    </div>
                  </div>
